...
function startWebSocket() {
        ...
        if (response != null) {
            ...
            if (response.msg_type === "proposal_open_contract") {
                if (response.proposal_open_contract.contract_id === lastTradeId) {
                    ...
                    // console.log('contract.is_sold - ', contract.is_sold);
    ...
    const stakeChange = (status) => {
        // console.log('last trade status - ', status);
    ...
    function resetParams() {
        ...
        let autoGeneratedTargetAmount = 0;
        let autoGeneratedAmountForTrading = 0;

        const targets = calculateTargets(fullAmount, fullAmount);
        ...
        if(targets.length > 0){
            ...
            // let autoGeneratedTargetAmount = Math.floor(fullAmount / 200) * 2;
            autoGeneratedTargetAmount = targets[0].target;
            autoGeneratedAmountForTrading = targets[0].stake;
            // let autoGeneratedAmountForTrading = ((fullAmount / 200) * 1.5).toFixed(2);
            
        }


        targetProfitInputElement.value > 0 ? targetAmount = targetProfitInputElement.value : targetAmount = autoGeneratedTargetAmount;
        initialStakeInputElement.value > 0 ? amountForTrading = initialStakeInputElement.value : amountForTrading = autoGeneratedAmountForTrading;
    ...
    function calculateTargets(capital, maxValue) {
        // console.log(capital, maxValue);
        const results = [];
        let rangeStart = 200;
        const increment = 200;
        ...
        let target = 0.5;
        let stake = 0.35;
        const targetIncrement = 0.5;
        const stakeIncrement = 0.35;
    

        // let target = 10;
        // let stake = 8;
        // const targetIncrement = 5;
        // const stakeIncrement = 5;
    


        while (rangeStart <= maxValue) {
            const rangeEnd = rangeStart + increment;
    
            if (capital >= rangeStart && capital < rangeEnd) {
                results.push({
