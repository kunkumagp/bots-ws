...
function tryEvenEntry(evenPercentage, lastDigits) {
    ...
    // Secondary check: compute last-10 digits even percentage
    const last10 = lastDigits.slice(-10);
    const evenCount10 = last10.filter((d) => d % 2 === 0).length;
    const even10Percentage = Number(((evenCount10 / last10.length) * 100).toFixed(2));

    // Require primary >=54 and secondary (last-10) >=60 to proceed
    if (evenPercentage < 54 || even10Percentage < 60) return false;
...
function tryOddEntry(oddPercentage, lastDigits) {
    ...
    // Secondary check: compute last-10 digits odd percentage
    const last10 = lastDigits.slice(-10);
    const oddCount10 = last10.filter((d) => d % 2 !== 0).length;
    const odd10Percentage = Number(((oddCount10 / last10.length) * 100).toFixed(2));

    // Require primary >=54 and secondary (last-10) >=60 to proceed
    if (oddPercentage < 54 || odd10Percentage < 60) return false;
